...
import { createExcerpt, getAllPages, EXCURSIONS_DATA } from '../services/contentService.ts';
import type { IPageData } from '../types';
import type { ViewState } from '../App';

interface ExcursionsHubPageProps {
  setView: React.Dispatch<React.SetStateAction<ViewState>>;
}

const ExcursionsHubPage: React.FC<ExcursionsHubPageProps> = ({ setView }) => {
  const [excursions, setExcursions] = useState<IPageData[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const finalExcursions = EXCURSIONS_DATA.map(excursionDef => {
          const pageContent = allPages.find(p => p.wp_id === excursionDef.wp_id);
          if (pageContent) {
            return {
              ...pageContent,
              title: excursionDef.title, 
              slug: excursionDef.slug,
            };
          }
          return null;
        }).filter((p): p is IPageData => p !== null);

        setExcursions(finalExcursions);

      } catch (e: any) {
        console.error("Critical error loading excursions:", e);
  ...
  if (loading) {
    return (
      <div className="text-center py-40 font-archival animate-pulse">
        ...
        <h2 className="text-xl text-[#2C3531]">Загружаем каталог экскурсий...</h2>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      <div className="text-center mb-16">
        <h1 className="text-4xl md:text-6xl font-expressive text-[#2C3531] mb-6">Экскурсии по Алтаю</h1>
        <p className="text-lg text-[#2C3531]/60 font-archival max-w-2xl mx-auto">
          Каталог однодневных и коротких экскурсионных программ.
        </p>
      </div>

      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {cards.map(excursion => (
          <div key={excursion.wp_id} onClick={() => setView({ page: 'excursionDetail', params: { excursionId: encodeURIComponent(excursion.slug) } })} className="block group h-full cursor-pointer">
...
export default ExcursionsHubPage;
