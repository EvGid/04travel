...
import { getAllPages, LOCATIONS_DATA, TOUR_SLUGS_SET, LOCATION_SLUGS_SET, EXCURSION_SLUGS_SET, SLUG_ALIASES } from '../services/contentService.ts';
...
const LocationDetailPage: React.FC<LocationDetailPageProps> = ({ locationId, setView }) => {
  const [page, setPage] = useState<IPageData | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const contentRef = useRef<HTMLDivElement>(null);

  const decodedSlug = decodeURIComponent(locationId || '');
  const currentIndex = LOCATIONS_DATA.findIndex(t => t.slug.toLowerCase() === decodedSlug.toLowerCase());

  const canGoBackInList = currentIndex > 0;
  const canGoForwardInList = currentIndex < LOCATIONS_DATA.length - 1;

  const handleGoBack = () => {
    if (canGoBackInList) {
      const prev = LOCATIONS_DATA[currentIndex - 1];
      setView({ page: 'locationDetail', params: { locationId: encodeURIComponent(prev.slug) } });
    }
  };

  const handleGoForward = () => {
    if (canGoForwardInList) {
      const next = LOCATIONS_DATA[currentIndex + 1];
      setView({ page: 'locationDetail', params: { locationId: encodeURIComponent(next.slug) } });
    }
  };
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        const locationDefinition = LOCATIONS_DATA[currentIndex];
        ...
        const foundPage = allPages.find(p => p.wp_id === locationDefinition.wp_id);

        if (foundPage) {
          setPage({ ...foundPage, title: locationDefinition.title });
        } else {
          console.error(`Контент для локации с wp_id=${locationDefinition.wp_id} (${locationDefinition.title}) не найден в JSON!`);
          setPage(null);
        }
