...
const actions = [
  ...
  {
    label: 'Профиль Max',
    href: 'https://max.ru/id41103048347_biz',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor">
...
const iconScales: { [key: string]: string } = {
  'Telegram': 'scale-[1.05]',
  'ВКонтакте': 'scale-[1.5]',
  'Профиль Max': 'scale-[1.12]',
};

const BookingActions: React.FC = () => {
  return (
    <div className="mt-16 pt-10 border-t border-gray-200/60">
      <h2 className="text-center text-3xl font-expressive mb-10 text-[#2C3531]">
          ...
          >
            <div className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center">
              {React.cloneElement(action.icon as React.ReactElement, {
                className: `w-full h-full text-gray-500 group-hover:text-[#4A5D4E] transition-colors ${iconScales[action.label] || ''}`
              })}
            </div>
          </a>
        ))}
